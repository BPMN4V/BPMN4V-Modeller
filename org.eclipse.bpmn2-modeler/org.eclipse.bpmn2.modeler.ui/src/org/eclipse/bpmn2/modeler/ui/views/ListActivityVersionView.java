...
public class ListActivityVersionView extends ViewPart {
	...
	private final class BpmnModelDragSourceLisener extends DragSourceAdapter {
		...
		public void dragStart(final DragSourceEvent event) {
			if (((IStructuredSelection) viewer.getSelection()).getFirstElement() instanceof TreeObject) {
				...
				TransactionalEditingDomain domain = BPMNToolBehaviorProvider.editor.getEditingDomain();
				domain.getCommandStack().execute(new RecordingCommand(domain) {
					@Override
					protected void doExecute() {
						IStructuredSelection iStructuredSelection = (IStructuredSelection) viewer.getSelection();
						TreeObject treeObject = (TreeObject) iStructuredSelection.getFirstElement();
						final IFeatureProvider fp = BPMNToolBehaviorProvider.featureProvider;
						CreateContext context=new CreateContext();
						context.setTargetContainer(BPMNToolBehaviorProvider.editor.getDiagramTypeProvider().getDiagram());
						context.setLocation(event.x, event.y);
						CreateActivityDrag c=new CreateActivityDrag(fp);
						c.setActivityId(treeObject.getName());
						c.setActivityName(treeObject.getParent().getName());
						 ob=c.create(context);
					}
				});
			
				
				//LocalSelectionTransfer.getTransfer().setSelection(new StructuredSelection(ob[0]));
			}
	...
	public void createPartControl(Composite parent) {
		...
		final ListActivityViewContentProvider contentProvider = new ListActivityViewContentProvider();
		...
		listener = new BpmnModelViewerSelectionListener3(viewer);
