...
public class WizardTarget2 extends WizardPage implements IConstants {
	...
	public static String activityNameTarget2;
	...
	public static Boolean afficheFinishT2 = false;
...
private String processStableT2;
	/**
	 * Constructor for SampleNewWizardPage.
	 * 
	 * @param pageName
	 */
	public WizardTarget2(ISelection selection) {
		super("wizardPageT2"); //$NON-NLS-1$
...
containerText.addSelectionListener(new SelectionListener(){
	...
	public void widgetSelected(SelectionEvent arg0) {
		...
		iddt2 = new Label(container, SWT.NONE);
		 idt2 =selectTaskId();
		iddt2.setText(idt2);
		UpdateTaskId(idt2);
		iddt2.setVisible(false);
		...
		nameText.addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent e) {
				System.out.println("Text after modification: "
						+ nameText.getText());
				nextText = true;
				activityNameTarget2 = nameText.getText();
				newInteraction = true;
				afficheFinishT2 = true;
	...
	public void select_activity_name() {
		...
		try {
			...
			// rootLibrary.executeXQuery("for $p in doc('Processes.xml')/Processes/Process for $i in $p/versions/version for $o in $i/activities for $a in doc('Activities.xml')/Activities/Activity for $l in $a/versions/version where $o/id_va=$l/id_v and $p/name='"+BPMNToolBehaviorProvider.name_process+"' return $a/name");
			// IterableIterator<? extends XhiveXQueryValueIf> result =
			// rootLibrary.executeXQuery("for  $p in fn:doc('Processes.xml')/Processes/Process  let $i:=<version></version> where $p/name='"+name+"' return insert nodes  $i into $p/versions");
		...
		try {
			...
			while (str0[k] != null) {
				...
				while (result1.hasNext()) {
					...
					/*
					 * j=0; while (str1[j]!=null)
					 * {System.out.println("str1"+str1[j]); j++;}
					 * 
					 * System.out.println("s1"+s1); String
					 * str[]=s1.split("<id_vd>"); for(int m=0;m<str.length;m++)
					 * {if (str[m]!="") {String str2[]=str[m].split("</id_vd>");
