...
public class ValidateVersionCollaboration extends AbstractCustomFeature {
	private String [] ProcessVersionActivitiesId=new String[20];

public ValidateVersionCollaboration(IFeatureProvider fp) {
...
public void execute( ICustomContext  context)
{
	...
	Collaboration c=	(Collaboration) handler.findElement(BPMNToolBehaviorProvider.id_v);
List <Participant> p=c.getParticipants();	
Iterator<Participant> i=p.iterator();
while (i.hasNext())
{Participant pi=i.next();
updateStateProcess(pi.getProcessRef().getId());
List<FlowElement> lf= pi.getProcessRef().getFlowElements();
...
updateActivityState(flow.getId());
		...
		JOptionPane.showMessageDialog(null, "The version of Collaboration "+ BPMNToolBehaviorProvider.id_v+" became Stable. This version can be instantiated", "Validate version of Process", JOptionPane.INFORMATION_MESSAGE);
	 ...
	 public void updateState(String id_v)
	 {
		 XhiveDriverIf driver = XhiveDriverFactory.getDriver("xhive://localhost:1235");  
		  ...
		  try {  
		    ...
		    IterableIterator<? extends XhiveXQueryValueIf>  result = rootLibrary.executeXQuery("for  $p in fn:doc('Collaboration.xml')/Collaborations/Collaboration for $i in $p/versions/version let $o:='Stable' where $i/id_v='"+id_v+"' return replace value of node $i/state with $o");
	...
	boolean trouve=false;
	 ...
	 public void updateStateProcess(String id_v)
	 {
		 XhiveDriverIf driver = XhiveDriverFactory.getDriver("xhive://localhost:1235");  
			int i=0;
			 if (driver.isInitialized()==false)
